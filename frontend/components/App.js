...
export default function App({ lang = 'en' }) {
  ...
  return (
    <div>
      <h2>
        {i18n[language].TEXT_HEADING_CREATE_ACCOUNT}
      ...
      <form onSubmit={onSubmit}>
        ...
        <div className="inputGroup">
          <label htmlFor="username">{i18n[language].LABEL_USERNAME}</label>
          <input id="username" name="username" onChange={onChange} value={values.username} type="text" placeholder={i18n[language].PLACEHOLDER_USERNAME} />
          {errors.username && <div className="validation">{errors.username}</div>}
        </div>

        <div className="inputGroup">
          <fieldset>
            <legend>{i18n[language].TEXT_FAV_LANG}</legend>
            <label>
              <input onChange={onChange} type="radio" name="favLanguage" value="javascript" checked={values.favLanguage == 'javascript'} />
              {i18n[language].TEXT_FAV_LANG_JS}
            </label>
            <label>
              <input onChange={onChange} type="radio" name="favLanguage" value="rust" checked={values.favLanguage == 'rust'} />
              {i18n[language].TEXT_FAV_LANG_RUST}
            </label>
          </fieldset>
        ...
        <div className="inputGroup">
          <label htmlFor="favFood">{i18n[language].LABEL_FAV_FOOD}</label>
          <select id="favFood" name="favFood" value={values.favFood} onChange={onChange}>
            <option value="">{i18n[language].TEXT_OPT_FAV_FOOD_1}</option>
            <option value="pizza">{i18n[language].TEXT_OPT_FAV_FOOD_2}</option>
            <option value="spaghetti">{i18n[language].TEXT_OPT_FAV_FOOD_3}</option>
            <option value="broccoli">{i18n[language].TEXT_OPT_FAV_FOOD_4}</option>
          </select>
        ...
        <div className="inputGroup">
          <label>
            ...
            {i18n[language].LABEL_ACCEPT_TERMS}
          </label>
          {errors.agreement && <div className="validation">{errors.agreement}</div>}
        </div>

        <div>
          <input type="submit" disabled={!submitAllowed} value={i18n[language].TEXT_SUBMIT} />
        </div>
      </form>
    </div>
  )
